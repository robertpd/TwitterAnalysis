...
public class IndexStatuses {
  ...
  public static final Analyzer ANALYZER = new TweetAnalyzer(Version.LUCENE_35, null);
  ...
  public static void main(String[] args) throws Exception {
    ...
    try 
    {
      while ((status = stream.next()) != null) 
      {
    	  ...
    	  //removed language identification, tika is very slow at this, painfully slow
    	  //        LanguageProfile lp = new LanguageProfile(status.getText());
    	  //        LanguageIdentifier li = new LanguageIdentifier(lp);
    	  //        if(li.getLanguage()=="en")
    	  //        {
    	  //test language, if not english then continue
    	  
    	  //hash tag indexer...
    	  //
    	  // have something like status.getHashes... return a string of just hashes
    	  // the analyser will tokenise the hashes. use a simple analyser that merely splits the string at a space
    	  
    	  cnt++;
    	  String createdAt = status.getCreatedAt();
    	  Document doc = new Document();
    	  doc.add(new Field(StatusField.TEXT.name, status.getText(), Store.YES, Index.ANALYZED, TermVector.YES));
    	  doc.add(new Field(StatusField.ID.name, status.getId() + "", Store.YES, Index.NO/*Index.NOT_ANALYZED_NO_NORMS*/));
