...
public class AsyncHtmlStatusBlockCrawler {
	...
	public static int protectedTweet=0;
	...
	private final ConcurrentSkipListMap<PairOfLongString, HtmlStatus> crawl = new ConcurrentSkipListMap<PairOfLongString, HtmlStatus>();
	...
	public void fetch() throws IOException {
		...
		try {
			...
			while ((line = data.readLine()) != null) {
				...
				if (cnt % TWEET_BLOCK_SIZE == 0 || cnt % 50 ==0) {
	...
	private class TweetFetcherHandler extends AsyncCompletionHandler<Response> {
		...
		public Response onCompleted(Response response) throws Exception {
			if (response.getStatusCode() >= 500) {
				if(response.getUri().getRawPath().contains("protected")){
		...
		private synchronized void retry() throws Exception {
			...
			if(url.contains("protected_redirect=true")){
				...
				// addition: skip protected tweets
